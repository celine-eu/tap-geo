# AGENTS.md - AI Agent Development Guide for tap-geo
...
This tap follows the Singer specification and uses the Meltano Singer SDK to extract data from Geo files.

### Key Components

1. **Tap Class** (`tap_geo/tap.py`): Main entry point, defines streams and configuration
1. **Client** (`tap_geo/client.py`): Handles API communication and authentication
1. **Streams** (`tap_geo/streams.py`): Define data streams and their schemas
...
1. Define stream class in `tap_geo/streams.py`
...
```bash
tap-geo --config config.json --discover
tap-geo --config config.json --catalog catalog.json
```
...
1. Update `config_jsonschema` in `tap_geo/tap.py`
...
```bash
# .env.example
TAP_JSONFILE_API_URL=https://api.example.com
TAP_JSONFILE_API_KEY=your_api_key_here
TAP_JSONFILE_BATCH_SIZE=100  # New setting
```
...
Before filing, search existing issues to avoid duplicates. Include the SDK version (`uv run tap-geo --version`), Python version, and a minimal reproduction case when reporting bugs.
